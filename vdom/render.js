...
var listenerKey = typeof Symbol !== 'undefined' ? Symbol('listener') : '__listener__'
...
function createHTML(vnode) {
  ...
  var temp = DOM.createElement(parent)
...
function createElement(vnode, eventNode, ns) {
  ...
  element = ns ?
    is ? DOM.createElementNS(ns, tag, {is: is}) : DOM.createElementNS(ns, tag) :
    is ? DOM.createElement(tag, {is: is}) : DOM.createElement(tag)
...
function createTagger(vnode, eventNode, ns) {
  var info = getVnodeTagger(vnode),
    subNode = Vnode.normalize(info.children)
  if (subNode != null) {
    var element = createNode(subNode, EventNode(info.tagger, eventNode), ns)
...
function setAttr(vnode, eventNode, key, old, value, ns) {
  ...
  if (nsLastIndex > -1 && key.substr(0, nsLastIndex) === "xlink") {
    ...
    if (vnode.tag === "input" && key === "value" && vnode.dom.value === value && vnode.dom === DOM.activeElement()) return
    element[key] = value
  }
  else {
...
function createListener() {
  return function eventHandler(event) {
    ...
    if (on && on[name]) invokeHandler(on[name], vnode.dom, eventHandler.eventNode, event)
  }
}
function invokeHandler(handler, dom, eventNode, event) {
  var msg = isArray(handler)
    ? invokeArrayHandler(handler, dom, event)
...
function init(tagger) {
  ...
  return function renderer(dom, vnodes) {
    ...
    dom.vnodes = vnodes
    if (DOM.activeElement() !== active) active.focus()
  }
}
