  ...
  describe('Task Monad', () => {
    describe('Functor instance', () => {
      jsv.property('identity', jsv.nat, a =>
        T.toPromise(T.bothPar(T.pure(a).map(x => x), T.pure(a)).map(tEquals)));
    ...
    describe('apFirst()', () => {
      jsv.property('keeping only the result of the first', jsv.nat, jsv.nat, (a, b) =>
        T.toPromise(T.apFirst(T.pure(a), T.pure(b)).map(x => deepEq(a, x)))
      );
    });

    describe('apSecond()', () => {
      jsv.property('keeping only the result of the second', jsv.nat, jsv.nat, (a, b) =>
        T.toPromise(T.apSecond(T.pure(a), T.pure(b)).map(x => deepEq(b, x)))
      );
    });
