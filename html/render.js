...
module.exports = function ($window) {
  ...
  function createNode(vnode, eventNode, ns) {
    var tag = vnode.tag, nodetype = typeof tag
    if (nodetype === 'string') {
      switch (tag) {
    ...
    if (isVnodeTagger(vnode)) return createTagger(vnode, eventNode, ns)
  ...
  function createTagger(vnode, eventNode, ns) {
    var tag = vnode.tag, tagger = tag.tagger, subNode = tag.vnode, subTag, subEventNode
    while (isVnodeTagger(subNode)) {
      subTag = subNode.tag
      if (typeof tagger !== 'object') tagger = [tagger, subTag.tagger]
      else tagger.push(subTag.tagger)
      subNode = subTag.vnode
    }
    subEventNode =
      { tagger: tagger
      , parent: eventNode }
    vnode.instance = Vnode.normalize(subNode)
    if (vnode.instance != null) {
      var element = createNode(vnode.instance, subEventNode, ns)
  ...
  function updateNode(parent, old, vnode, eventNode, nextSibling, recycling, ns) {
    ...
    else if (isVnodeTagger(old) && isVnodeTagger(vnode)) updateTagger(parent, old, vnode, eventNode, nextSibling, recycling, ns)
    else if (isVnodeThunk(old) && isVnodeThunk(vnode)) updateThunk(parent, old, vnode, eventNode, nextSibling, recycling, ns)
    else {
  ...
  function updateTagger(parent, old, vnode, eventNode, nextSibling, recycling, ns) {
    ...
    while(isVnodeTagger(subNode)) {
      nesting = true
      var subNodeTag = subNode.tag
      if (typeof tagger !== 'object') {
        tagger = [tagger, subNodeTag.tagger]
      } else {
        tagger.push(subNodeTag.tagger)
      }
      subNode = subNodeTag.vnode
    }
    var oldSubNode = oldTag.subNode, oldTagger = old.tagger
    while (isVnodeTagger(oldSubNode)) {
      nesting = true
      var oldSubNodeTag = oldSubNode.tag
      if (typeof oldTagger !== 'object') {
        oldTagger = [oldTagger, oldSubNodeTag.tagger]
      } else {
        oldTagger.push(oldSubNodeTag.tagger)
      }
      oldSubNode = oldSubNodeTag.vnode
    }
    ...
    if (vnode.instance != null) {
      if (old.instance == null) insertNode(parent, createNode(vnode.instance, subEventNode, ns), nextSibling)
      else updateNode(parent, old.instance, vnode.instance, subEventNode, nextSibling, recycling, ns)
      vnode.dom = vnode.instance.dom
